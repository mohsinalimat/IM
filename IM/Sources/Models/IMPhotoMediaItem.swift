...
open class IMPhotoMediaItem: IMMediaItem {
    ...
    open override func copy(with zone: NSZone?) -> Any {
        if let ci = self.image?.cgImage {
            let copy = type(of: self).init(image: UIImage(cgImage: ci))
            copy.appliesMediaViewMaskAsOutgoing = self.appliesMediaViewMaskAsOutgoing
            return copy
        }
        let copy = type(of: self).init(image: self.image)
        copy.appliesMediaViewMaskAsOutgoing = self.appliesMediaViewMaskAsOutgoing
        return copy
    }
}
